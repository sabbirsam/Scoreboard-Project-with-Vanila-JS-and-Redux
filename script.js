  ...
  function reducer(state = initialState, action) {
    ...
    switch (action.type) {
      ...
      case "REMOVE_MATCH":
          ...
          const updatednewMatches = state.matches.filter(match => match.id !== matchIdToRemove);
          const updatedMatchesWithIds = updatednewMatches.map((match, index) => {
            return {
              ...match,
              id: index + 1
            };
      ...
      case "RESET":
        // return initialState;
        const resetMatches = state.matches.map(match => ({ ...match, result: 0 }));
        return {
          ...state,
